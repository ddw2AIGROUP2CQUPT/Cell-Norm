...
def segment_image(sam_generator, image_rgb: np.ndarray):
    ...
    try:
        ...
        for m in masks:
            if "segmentation" in m and m["segmentation"].shape == (h, w):
                combined = np.maximum(combined, m["segmentation"].astype(np.uint8) * 255)
        return image_rgb, combined
    except Exception as e:
        print(f"分割失败: {e}")
        return image_rgb, None
...
def _prepare_cells_redistribute(
    ...
    if sam_generator is None:
        return _skip("no_sam")
    _, mask = segment_image(sam_generator, rgb_work)
    if mask is None:
...
def run(
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        ...
        with tqdm(total=len(images), desc="Processing", unit="img", dynamic_ncols=True) as pbar:
            for i, path in enumerate(images):
                ...
                n_skipped = sum(skip_counts.values())
                pbar.set_postfix(written=ok_files, src_ok=ok_sources_full, skipped=n_skipped, refresh=False)
                pbar.update(1)
                if prep is None:
                    if n_skipped % 100 == 0 and n_skipped > 0:
                        _print_skip_summary("中间统计", pbar.n, ok_sources_full, skip_counts)
                    continue
    ...
    _print_skip_summary("单GPU", len(images), ok_sources_full, skip_counts)
    print(f"共写出 {ok_files} 个 png（含对应 xml+layout）")
...
def main():
    ...
    parser.add_argument(
        "--image-folder",
        type=str,
        default="/home/ubuntu/san/TCT/DataSets/v4_1024/images/test",
        help="图片目录（建议 640×640；否则默认缩放到 640）",
    )
    parser.add_argument(
        "--xml-folder",
        type=str,
        default="/home/ubuntu/san/TCT/DataSets/v4_1024/labels/test",
        help="XML 目录",
    )
    parser.add_argument(
        "--output-image",
        type=str,
        default="/home/ubuntu/san/TCT/DataSets/v4_1024/1024_to_640_1xdata_5_20/test/images",
        help="输出图片目录",
    )
    parser.add_argument(
        "--output-xml",
        type=str,
        default="/home/ubuntu/san/TCT/DataSets/v4_1024/1024_to_640_1xdata_5_20/test/labels",
